...
argument-hint: [path to the one file to implement, or blank to infer from the plan]
---
...
1. An active spec exists under `docs/specs/` with a `## Success criteria`
   section.
2. `/plan` has run and the target is **one** file. If the plan touches more
   than one, say so and stop — split the spec instead.
...
- A reminder that a green gate is necessary and not sufficient — `/review`
  still runs against the spec afterward.
