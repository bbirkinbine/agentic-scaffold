...
Aim the hostility at what a machine cannot check. `/review-check` has already
run the project's linter, formatter, and type checker over this diff, and CI
runs them again — re-deriving a finding they block spends your budget proving
something the human already has. Missing annotations, import order, and
formatting are gate territory; raise them only where the gate is configured not
to catch them. For generated files, review the authored source and verify the
output is synchronized. Still inspect generated and vendored changes for
security, licensing, provenance, public-repo hygiene, and client compatibility.
Logic, spec match, design, and the failure modes below are where a hostile read
pays.
...
5. **Don't-touch zones.** Did the diff cross protected paths listed in `AGENTS.md` without explicit justification?
6. **Naming + docstrings.** Do new symbols obscure intent? Tautological docs?
